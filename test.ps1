...
#   1) engine  2) api  3) ruff. El SNAPSHOT DORADO es sagrado: rojo => NO desplegar.
...
# Asegura pytest (no está en las deps del motor).
...
Write-Host "`n================  1/3  engine (aleph_engine — incl. harness dorado)  ================" -ForegroundColor Cyan
Push-Location (Join-Path $PSScriptRoot "engine"); & $Py -m pytest; if ($LASTEXITCODE -ne 0) { $fail = 1 }; Pop-Location

Write-Host "`n================  2/3  api (aleph_api)  ================" -ForegroundColor Cyan
Push-Location (Join-Path $PSScriptRoot "api"); & $Py -m pytest; if ($LASTEXITCODE -ne 0) { $fail = 1 }; Pop-Location

Write-Host "`n================  3/3  ruff (engine + api)  ================" -ForegroundColor Cyan
& $Py -m ruff --version 2>$null | Out-Null
if ($LASTEXITCODE -eq 0) {
  & $Py -m ruff check engine api; if ($LASTEXITCODE -ne 0) { $fail = 1 }
} else {
...
Write-Host ""
if ($fail -eq 0) {
  Write-Host "OK  TODO VERDE — seguro para desplegar (API: api/DEPLOY.md · web: push a Vercel)" -ForegroundColor Green
} else {
